            ...
            throws IOException, ServletException
    {
        ...
        else
        {
            ...
            if(alreadyLoginId != null){ // 이미 login된 아이디가 있다. 그런데 social로그인을 했다. 이미 있는 회원인데 소셜 로그인을 했다는 것은 소셜 로그인 연결을 했다는 의미다.
                ...
                joinUser.setUserConnection(socialLoginInfo.getUserConnection());
                userService.addUserConnection(joinUser.getUserConnection());

                // 이미 가입되어 있는데 소셜로그인을 하는 경우는 사용자 정보보기 밖에 없다.
                res.sendRedirect(req.getContextPath() + "/users/info");
                ...
                res.sendRedirect(req.getContextPath() + "/users/alreadySocialJoin");
                return;
            }else {
                ...
                userConnection.addUser(joinUser);

                userService.addUserConnection(userConnection);
